...
BUCKET=$(aws cloudformation describe-stacks \
  ...
  jq '.Stacks[0].Outputs | map(select(.OutputKey == "NextjsBucketName"))[0].OutputValue' | \
  tr -d \")
...
then
  echo "no changes to nextjs from ${bucket_sha} to ${GITHUB_SHA}"
  echo "not going any further with nextjs this build"
  exit 0
fi
